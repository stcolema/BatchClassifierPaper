...
for (j in seq(1, n_hyperparam_combinations)) {
  ...
  for (i in 1:n_chains) {
    ...
    if (i == 1 & j == 1) {
      batch_acceptance_df <- mvt_batch_acceptance_df # rbind(mvn_batch_acceptance_df, mvt_batch_acceptance_df)
      cluster_acceptance_df <- mvt_cluster_acceptance_df # rbind(mvn_cluster_acceptance_df, mvt_cluster_acceptance_df)
...
used_int <- seq(eff_burn + 1, R / thin)

alloc <- mvt_samples[[3]]$samples[used_int, ]

psm <- mdiHelpR::makePSM(alloc)
my_cl <- mcclust::maxpear(psm, max.k = 2)$cl

my_cl_2 <- rep(1, N)
my_cl_2[which(my_cl == 1)] <- 2

# Inferred datasets
mvt_inferred_data <- rowMeans(mvt_samples[[3]]$batch_corrected_data[, , -c(1:eff_burn)], dims = 2) %>%
...
library(patchwork)
p_raw_data / p_inferred


ggsave(paste0(save_dir, "unsupervisedMVTInferredData.png"), plot = p_inferred, width = 5, height = 6)
